...
async def scrape_price(url: str, css_selector: str):
    ...
    try:
        browser, ctx, page = await _get_page_content(url)
        try:
...
async def scrape_image(url: str, image_selector: str | None = None) -> str | None:
    ...
    1. via image_selector si fourni (attribut src ou srcset)
...
def _parse_price(text: str) -> float | None:
    if not text:
        return None
    # Nettoyage : on garde chiffres , . espaces
    t = text.replace("\xa0", " ").replace(" ", "")
    # cas "1 234,56" ou "1.234,56"
    m = re.search(r"(\d[\d.,]*)", t)
    if not m:
        return None
    num = m.group(1)
    # si virgule présente => décimale FR
    if "," in num:
